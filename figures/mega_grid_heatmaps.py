...
def make_heat_map(df, model, prompt, plot_dir, *, log_y=True):
    ...
    print(df.head(2))
    # if prompt == "cot":
    #     df["output_numeric"] = df["output_binary"]
    ...
    # np.save(f"similarity/temp0/{prompt}/{model}_mean_vector.npy", mean)
    # output_path = f"similarity/temp0/{prompt}/{model}_mean_matrix.npy"
    # os.makedirs(os.path.dirname(output_path), exist_ok=True)
    # np.save(output_path, mean)
    # print(f"Saved matrix with shape {mean.shape} to {output_path}")
    ...
    out_pdf = os.path.join(plot_dir, f"{model}_log.pdf")

    # Save all formats (dpi is ignored by vector formats, so we can omit it for them)
...
def assemble_grid(
    base_dir,  # =f"results_temp1_OR/",
    out_png,  # ="results_temp1_OR/all/combined_prompts_models.png",
):
    ...
    cax = fig.add_axes([0.4, 0.15, 0.3, 0.025])
    ...
    #     0.55,
    #     0.93,
    #     "Distance vs Distance ('Walk in a safe neighbourhood)",  # "Baseline vs Chain-of-Thought"
    #     ha="center",
    #     va="top",
    #     fontsize=15,
    # )
    ...
    plt.close(fig)
    print(f"mega-grid saved to: {out_base}.png, .svg, and .pdf")
...
if __name__ == "__main__":
    ...
    for prompt in PROMPTS:
        ...
        out_dir = f"new_figures/llama_scale/{prompt}/"
    ...
    assemble_grid(
        base_dir="new_figures/llama_scale/",
        out_png="new_figures/llama_scale/combined",
    )
    # assemble_grid()
