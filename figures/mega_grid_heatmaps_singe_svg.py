...
ROW_SUBTITLES = {
    ...
    # "temp=1": "Appointment postponement [min]",
...
def assemble_grid(all_data_dict, prompts_to_plot, out_base):
    ...
    plt.subplots_adjust(
        ...
        bottom=0.35, 
        hspace=0.25, 
        wspace=0.05, # <-- DECREASED from 0.12 to reduce spacing between heatmaps
    )
    ...
    for r, prompt in enumerate(prompts_to_plot):
        for c, model in enumerate(MODEL_ORDER):
            ...
            # Set the string Y label (e.g., "Time") for the first column only
            if c == 0:
                ax.set_ylabel(ROW_NAMES.get(prompt, prompt), fontsize=15, weight="bold")

    # --- 3. X label below the grid ---
    for r, prompt in enumerate(prompts_to_plot):
    ...
    fig.text(
        0.535, # Centered with the plots
        1,  # Placed high up on the figure
        "Model Scale: 1B, 3B, 8B, 70B", 
        ha="center", 
        va="center",
        fontsize=16,
        weight="bold", # "bold"
    ...
    # --- Main Y Label ---
    fig.text(
        0.075, # <-- Slightly higher X so it sits on the "top" (right side)
        0.62, 
        "Reward Offered", 
        rotation="vertical",
        va="center",
    ...
    fig.text(
        0.095, # <-- Lower X so it sits on the "bottom" (left side)
        0.62, 
        "[in Euro, log scale]", 
        rotation="vertical",
        va="center",
        ...
        weight="normal", # <-- Un-bolding units often looks much cleaner, but you can change to "bold" if preferred
    )

    # --- 4. Colorbar positioning ---
    cax = fig.add_axes([0.385, 0.1, 0.30, 0.0375]) 
...
if __name__ == "__main__":
    ...
    for prompt in PROMPTS:
        pkl = f"results/{prompt}/rev_21x21x5_llama_scale/data_log.pkl"
        out_dir_individual = f"new_figures/llama_scale/{prompt}/"
        ...
        try:
            ...
            # 1. Generate individual small SVG/PNG images
            ...
            # 2. Generate the combined vector grid FOR THIS PROMPT ONLY
            print(f"\n--- Generating combined vector grid for {prompt} ---")
            assemble_grid(
                all_data_dict=master_data,
                prompts_to_plot=[prompt], # <-- Only plot the current prompt
                out_base=f"new_figures/llama_scale/{prompt}/combined", # <-- Save in the prompt's folder
            )
