...
def assemble_grid(all_data_dict, prompts_to_plot, out_base):
    ...
    for r, prompt in enumerate(prompts_to_plot):
        ...
        fig.text(
            center_x, 
            pos_left.y0 - 0.027, # Placed just below the X-axis tick numbers
    ...
    fig.text(
        0.53, 
        0.97,  
        "Inconvenience-Reward LLM Trade-Offs", 
        ha="center", 
        va="center",
        fontsize=24,
        weight="bold", 
    ...
    fig.text(
        ...
        0.5,  # Centered vertically for the whole multi-row figure
        "Reward Offered", 
        rotation="vertical",
        va="center",
        ha="center", 
    ...
    cax = fig.add_axes([0.392, 0.025, 0.30, 0.02]) 
...
if __name__ == "__main__":
    ...
    for prompt in PROMPTS:
        pkl = f"results/{prompt}/rev_21x21x5_new_all/data_log.pkl"
        out_dir_individual = f"new_figures/main/{prompt}/"
    ...
    assemble_grid(
        ...
        prompts_to_plot=PROMPTS, # <-- Passes all uncommented prompts to make the rows
        out_base="new_figures/main/combined_all_prompts", # <-- Saves to a general folder
    )